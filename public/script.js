...
// ==========================================
// COURSES DATA (Strength & Powerlifting Only)
// ==========================================
const COURSES = [
    {
        id: 'course_powerlifting_foundation',
        title: 'Powerlifting Foundation: Squat, Bench & Deadlift',
        ...
        color: '#ef4444',
        description: 'Master the big three lifts from the ground up. Technique breakdowns, RPE-based programming, and competition-day strategy for first-time powerlifters.',
        highlights: ['SBD technique video guide', 'RPE-based 12-week block', 'Meet-day preparation', 'Lifetime access']
    },
    {
        id: 'course_strength_programming',
        title: 'Science of Strength Programming',
        ...
        icon: 'fa-brain',
        color: '#f59e0b',
        description: 'Learn to write your own strength programs using periodisation, RPE, volume landmarks, and fatigue management. Stop following cookie-cutter plans.',
        highlights: ['Periodisation models (linear, DUP)', 'Volume & intensity landmarks', 'Deload protocols', 'Program templates included']
    },
    {
        id: 'course_olympic_lifting',
        title: 'Olympic Weightlifting: Snatch & Clean & Jerk',
        ...
        icon: 'fa-trophy',
        color: '#8b5cf6',
        description: 'Full technique-to-competition pathway for Olympic weightlifting. Covers positional drills, pulling mechanics, and competition preparation for both lifts.',
        highlights: ['Snatch & C&J breakdown', 'Positional drills library', 'Competition block programming', 'Coaching cues & corrections']
    },
    {
        id: 'course_athlete_development',
        title: 'Athlete Development & Coaching Certification',
        ...
        color: '#10b981',
        description: 'Become a strength & conditioning coach. Covers needs analysis, program design, athlete monitoring, recovery systems, and practical coaching frameworks.',
        highlights: ['Strength & conditioning principles', 'Athlete assessment tools', 'Fatigue management systems', '1-on-1 mentorship sessions']
    }
];
...
const app = {
            ...
            if (app.authMode === 'login') {
                ...
                if (!res.ok) {
                    ...
                    if (data.error && data.error.toLowerCase().includes('not found')) {
                        ...
                        const msgEl = document.getElementById('auth-redirect-message');
                        if (msgEl) {
                            msgEl.textContent = 'No account found. Create your athlete profile to continue.';
                            msgEl.classList.remove('hidden');
                        }
                        return;
                    }
        ...
        if (loginBtn && signupBtn && logoutBtn) {
            if (user) {
                ...
                if (goalBadge && user.profile) {
                    const sportLabels = {
                        powerlifting: 'Powerlifting', strength_training: 'Strength',
                        olympic_weightlifting: 'Weightlifting', mma: 'MMA',
                        muscle_building: 'Muscle Building', other: 'Athlete'
                    };
                    const sport = user.profile.sport || user.profile.goal || 'other';
                    goalBadge.textContent = user.proStatus === 'active' ? 'PRO' : (sportLabels[sport] || 'Athlete');
    ...
    // New multi-step onboarding
    _obSport: null,

    selectSport: (btn) => {
        document.querySelectorAll('.sport-card').forEach(c => c.classList.remove('selected'));
        btn.classList.add('selected');
        app._obSport = btn.dataset.sport;
        const nextBtn = document.getElementById('ob-next-1');
        if (nextBtn) nextBtn.disabled = false;

        // Populate lift goals for step 3
        const liftGoalsContainer = document.getElementById('ob-lift-goals');
        if (liftGoalsContainer) {
            const lifts = {
                powerlifting: ['Squat', 'Bench Press', 'Deadlift'],
                strength_training: ['Squat', 'Bench Press', 'Deadlift', 'Overhead Press'],
                olympic_weightlifting: ['Snatch', 'Clean & Jerk'],
                mma: ['Squat', 'Deadlift', 'Overhead Press'],
                muscle_building: ['Bench Press', 'Squat', 'Barbell Row'],
                other: ['Squat', 'Bench Press', 'Deadlift']
            };
            const sport = btn.dataset.sport;
            const sportLifts = lifts[sport] || lifts['other'];
            liftGoalsContainer.innerHTML = sportLifts.map(lift => `
                <div class="lift-goal-row">
                    <label>${lift}</label>
                    <div class="lift-goal-inputs">
                        <input type="number" class="form-control" placeholder="Current (kg)" data-lift="${lift}" data-type="current" step="2.5" min="0">
                        <span class="lift-goal-arrow">→</span>
                        <input type="number" class="form-control" placeholder="Target (kg)" data-lift="${lift}" data-type="target" step="2.5" min="0">
                    </div>
                </div>`).join('');
        }
    },
    ...
    obNextStep: (step) => {
        if (step === 1) {
            ...
            if (!app._obSport) { app.showToast('Please select a sport first.', 'danger'); return; }
    ...
    handleOnboardingGoalChange: () => {}, // Legacy stub - no longer used

    handleOnboarding: () => {
        const sport = app._obSport || 'other';
        const weight = document.getElementById('ob-weight') ? document.getElementById('ob-weight').value : '';
        const height = document.getElementById('ob-height') ? document.getElementById('ob-height').value : '';
        const experience = document.getElementById('ob-experience') ? document.getElementById('ob-experience').value : 'intermediate';
        const injuries = document.getElementById('ob-injuries') ? document.getElementById('ob-injuries').value : 'none';
        const sleep = document.getElementById('ob-sleep') ? document.getElementById('ob-sleep').value : 'Good';
        const targetBW = document.getElementById('ob-target-bw') ? document.getElementById('ob-target-bw').value : '';
        const compDate = document.getElementById('ob-comp-date') ? document.getElementById('ob-comp-date').value : '';

        // Collect lift goals
        const liftGoals = {};
        document.querySelectorAll('#ob-lift-goals input').forEach(input => {
            const lift = input.dataset.lift;
            const type = input.dataset.type;
            if (lift && input.value) {
                if (!liftGoals[lift]) liftGoals[lift] = {};
                liftGoals[lift][type] = parseFloat(input.value);
            }
        });

        const profile = {
            sport,
            goal: ['powerlifting','olympic_weightlifting'].includes(sport) ? 'competition' : 'normal',
            weight: weight || null,
            height: height || null,
            experience,
            injuries,
            sleep,
            targetBodyweight: targetBW || null,
            competitionDate: compDate || null,
            liftGoals
        };

        // Set initial recovery from onboarding sleep
        store.saveRecovery({ sleep: sleep, soreness: 'Low' });

        // Save lift goals as app goals
        Object.entries(liftGoals).forEach(([lift, vals]) => {
            if (vals.target) store.saveGoal({ exercise: lift, target: vals.target });
        });

        // Save initial bodyweight
        if (weight && store.getBodyMetrics().length === 0) {
    ...
    renderDashboard: () => {
        ...
        const user = store.getUser();
        const recovery = store.getRecovery();
        const sport = user && user.profile && user.profile.sport ? user.profile.sport : null;
        const goal = user && user.profile ? user.profile.goal : 'normal';

        // Route competition athletes to competition dashboard
        if (sport === 'powerlifting' || sport === 'olympic_weightlifting' || goal === 'competition') {
    ...
    _renderNormalDashboard: (container, recovery) => {
        ...
        const user = store.getUser();
        const sport = user && user.profile && user.profile.sport ? user.profile.sport : 'strength_training';
        const sportLabels = {
            powerlifting: 'Powerlifting', strength_training: 'Strength Training',
            olympic_weightlifting: 'Weightlifting', mma: 'MMA',
            muscle_building: 'Muscle Building', other: 'Strength'
        };
        const sportLabel = sportLabels[sport] || 'Strength';

        // Streak calculation
        const streak = (() => {
            let s = 0;
            const today = new Date(); today.setHours(0,0,0,0);
            const days = [...new Set(workouts.map(w => new Date(w.date).toDateString()))].map(d => new Date(d)).sort((a,b) => b-a);
            for (let i = 0; i < days.length; i++) {
                const expected = new Date(today); expected.setDate(today.getDate() - i);
                if (days[i].toDateString() === expected.toDateString()) s++; else break;
            }
            return s;
        })();

        // Weekly volume
        const weeklyVol = Math.round(workouts.filter(w => (new Date() - new Date(w.date)) / (1000*60*60*24) <= 7).reduce((s,w) => s + store.calculateVolume(w), 0));
        ...
        // Intelligence alerts
        let alertsHTML = '';
        if (recovery.sleep === 'Poor') alertsHTML += '<div class="intel-alert warning"><i class="fa-solid fa-moon"></i> Poor sleep detected. Consider reducing intensity today.</div>';
        if (recovery.soreness === 'High') alertsHTML += '<div class="intel-alert danger"><i class="fa-solid fa-triangle-exclamation"></i> High soreness. Consider a deload or light session.</div>';
        if (thisWeek === 0 && totalWorkouts > 0) alertsHTML += '<div class="intel-alert info"><i class="fa-solid fa-calendar"></i> No workouts logged this week. Stay consistent.</div>';
        if (streak >= 5) alertsHTML += `<div class="intel-alert success"><i class="fa-solid fa-fire"></i> ${streak}-day training streak. Elite consistency.</div>`;
        ...
        container.innerHTML = `
            ...
            <div class="dashboard-stats-row">
                ...
                <div class="stat-card glass-card">
                    ...
                    <div class="stat-value">${weeklyVol > 1000 ? (weeklyVol/1000).toFixed(1)+'k' : weeklyVol}</div>
            ...
            ${alertsHTML ? `<div class="intel-alerts-row">${alertsHTML}</div>` : ''}
            ...
            <div class="dashboard-grid">
                ...
                <div class="dashboard-card glass-card">
                    ...
                    <button class="btn btn-outline btn-block mt-3" style="font-size:0.85rem; padding:0.5rem;" onclick="app.navigateAppPage('recovery')">Update Recovery</button>
                </div>

                <!-- Smart Recommendation Card -->
                <div class="dashboard-card glass-card recommendation-card primary-border">
                    <div class="card-header-flex">
                        <h3><i class="fa-solid fa-brain"></i> Smart Targets</h3>
                ...
                <div class="dashboard-card glass-card list-card">
                    ...
                    <button class="btn btn-outline btn-block mt-3" style="font-size:0.85rem; padding:0.5rem;" onclick="app.navigateAppPage('workout')">+ Log Workout</button>
                </div>
            </div>
        `;
    },
    ...
    _renderCompetitionDashboard: (container, recovery) => {
        const user = store.getUser();
        const sport = user && user.profile ? user.profile.sport : 'powerlifting';
        const sportLabels = {
            powerlifting: 'Powerlifting', olympic_weightlifting: 'Weightlifting', other: 'Competition'
        };
        const compLabel = sportLabels[sport] || 'Competition';
        const profileWeight = (user && user.profile && user.profile.weight) ? user.profile.weight : '—';
        const profileHeight = (user && user.profile && user.profile.height) ? user.profile.height : '—';
        const workouts = store.getWorkouts();
        const totalWorkouts = workouts.length;
        const thisWeek = workouts.filter(w => {
            const d = new Date(w.date);
        ...
        // Key lifts for this sport
        const sportKeyLifts = {
            powerlifting: ['Squat', 'Bench Press', 'Deadlift'],
            olympic_weightlifting: ['Snatch', 'Clean & Jerk'],
            other: ['Squat', 'Bench Press', 'Deadlift']
        };
        let keyLifts = sportKeyLifts[sport] || sportKeyLifts['other'];
    ...
    renderProgram: () => {
        ...
        const user = store.getUser();
        const sport = user && user.profile && user.profile.sport ? user.profile.sport : 'normal';
        // Map sport to program key
        const sportToProgramKey = {
            powerlifting: 'powerlifting',
        ...
        const programKey = sportToProgramKey[sport] || 'normal';
        const program = PROGRAMS[programKey] || PROGRAMS['normal'] || PROGRAMS['other'];
        if (!program) {
            container.innerHTML = '<div class="empty-state glass-card">No program available for your sport yet.</div>';
            return;
        }
        ...
        html += '<button class="btn btn-primary btn-block mt-4" onclick="app.navigateAppPage(\'workout\')"><i class="fa-solid fa-dumbbell"></i> Log Today\'s Workout</button>';
    ...
    ensurePaymentDeviceTools: (paymentView) => {
        if (!paymentView) return;
        const qrShell = paymentView.querySelector('.payment-qr-shell');
        const appGrid = paymentView.querySelector('.payment-app-grid');

        if (qrShell && !paymentView.querySelector('#payment-device-note')) {
            qrShell.insertAdjacentHTML('afterend', '<div id="payment-device-note" class="payment-device-note" aria-live="polite"></div>');
        }

        if (appGrid) {
            const keys = ['gpay', 'phonepe', 'paytm', 'generic'];
            appGrid.querySelectorAll('.upi-app-btn').forEach((link, index) => {
                link.setAttribute('data-upi-app', link.getAttribute('data-upi-app') || keys[index] || 'generic');
            });
    ...
    ensurePaymentView: () => {
        ...
        if (paymentView) {
            app.ensurePaymentDeviceTools(paymentView);
            return paymentView;
        }
        ...
        app.ensurePaymentDeviceTools(wrapper);
        return wrapper;
    },
    ...
    renderPaymentPage: (item, payment) => {
        ...
        const deviceNote = document.getElementById('payment-device-note');
    ...
    copyUPIId: () => {
        ...
        _copyTextToClipboard(upiId).then(() => {
            app.showToast('UPI ID copied to clipboard!');
        }).catch(() => {
            app.showToast(`${UPI_CONFIG.vpa} - copy manually.`);
        });
